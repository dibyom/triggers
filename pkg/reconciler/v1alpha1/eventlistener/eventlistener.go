...
func (r *Reconciler) FinalizeKind(ctx context.Context, el *v1alpha1.EventListener) pkgreconciler.Event {
	...
	if len(cfgs) != 1 {
		logger.Info("Not deleting logging config map since more than one EventListener present in the namespace %s", el.Namespace)
		return nil
	}
	// only one EL left
	lastEL := cfgs[0]
...
func (r *Reconciler) reconcileDeployment(logger *zap.SugaredLogger, el *v1alpha1.EventListener) error {
	...
	err := r.reconcileLoggingConfig(logger, el)
	if err != nil {
		logger.Error(err)
		return err
	}
